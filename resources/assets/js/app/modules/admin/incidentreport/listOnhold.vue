        ...
        <div>
            <v-client-table :data="rows" :columns="columns" :options="options">
                <template slot="actions" slot-scope="props">
                    <div v-if="props.row.status_id == 3" style="width:280px;">
                        ...
                        <button class='btn btn-warning btn-xs'
                            ...
                            v-if="rule.incident_report.child_rules.in_progress.child_rules.view.is_enable
...
<script>
    ...
    export default {
        ...
        created(){
            if (this.$route.hash == '#on-hold' || this.$route.hash === "") {
                this.getList();
            }
            this.$emit("update", this.updatePagination);

            this.$bus.$on('updateList', this.getList);
            this.$bus.$on('onholdList', this.getList);
        ...
        methods:{
            ...
            getList: function () {
                this.isLoading = true
                this.$constants.Incident_API.get("/status/incident/"+3)
                .then(response => {
            ...
            viewEditNoteRow: function (props_row, button_action) {
                ...
                if(props_row){
                    ...
                    if (props_row.complainant.offense) {
                        ...
                        if (props_row.progression_occurence[p].first_occurrence != nul
           ...
           eventGenerateNte: function (complainant_id, respondent_user, is_generate_nte_invalid_ir, date_hearing, initial_irr_id) {
                ...
                && initial_irr_id == 8) {//absolved
                    getTitle = 'Are you sure you want to Generate NTE ' + titleDL + ' PDF file?';

                } else if (this.role == "Super Admin Access" || this.role == "HR Admin Access" || this.role == "HR Access") {
