    ...
    <div>
        ...
        <div v-if="categoriesProps">
            <div class="pull-left" style="">
                <h4>Operations Processes, System and Guidelines</h4>
            </div>
            <button-add
                @add="addEditRow()"
                thingToAdd="Offense"
            ...
            <hr style="width:100%"/>
                <v-client-table :data="rows" :columns="columns" :options="options">
                        ...
                        <template slot="first_instance" slot-scope="props">
                            <div style="width: auto;" v-html="(props.row.instance && 
                            props.row.instance.first_instance) ? 
                            props.row.instance.first_instance.name : ''"/>
                        </template>
                        <template slot="second_instance" slot-scope="props">
                           <div style="width: auto;" v-html="(props.row.instance && 
                           props.row.instance.second_instance) ? 
                           props.row.instance.second_instance.name : ''"/>
                        </template>
                        <template slot="third_instance" slot-scope="props">
                            <div style="width: auto;" v-html="(props.row.instance && 
                            props.row.instance.third_instance) ? 
                            props.row.instance.third_instance.name : ''"/>
                        </template>
                        <template slot="fourth_instance" slot-scope="props">
                            <div style="width: auto;" v-html="(props.row.instance && 
                            props.row.instance.fourth_instance) ? 
                            props.row.instance.fourth_instance.name : ''"/>
                        </template>
                        <template slot="fifth_instance" slot-scope="props">
                            <div style="width: auto;" v-html="(props.row.instance && 
                            props.row.instance.fifth_instance) ? 
                            props.row.instance.fifth_instance.name : ''"/>
                        </template>
                        <template slot="sixth_instance" slot-scope="props">
                            <div style="width: auto;" v-html="(props.row.instance && 
                            props.row.instance.sixth_instance) ? 
                            props.row.instance.sixth_instance.name : ''"/>
                        </template>
                        <template slot="seventh_instance" slot-scope="props">
                            <div style="width: auto;" v-html="(props.row.instance && 
                            props.row.instance.seventh_instance) ? 
                            props.row.instance.seventh_instance.name : ''"/>
                        </template>
                </v-client-table>
...
<script>
    ...
    export default {
        ...
        created(){
            ...
            if (this.$route.hash == '#operations-processes-system-and-guidelines') {
                this.getList();
            }
            this.$bus.$on('categoryList23', this.getList);
            this.columns = (this.offense_data.category_rule.edit.is_enable || 
            this.offense_data.category_rule.archive.is_enable) ?
            [
                'actions',
                'offense',
                'gravity',
