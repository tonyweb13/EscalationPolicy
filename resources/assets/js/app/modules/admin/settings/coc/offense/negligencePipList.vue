...
<script>
    ...
    export default {
        ...
        created(){
            ...
            if (this.$route.hash === '#negligence-pip') {
                this.getList();
            }
            this.$bus.$on('categoryList12', this.getList);
            ...
            getList: function() {
                ...
                this.$constants.Coc_API.get("/offense/category/"+this.offense_data.category_id)
                    .then(response => {
                        ...
                        if(this.rows[0].category.name){
                            this.categoryTitle = "List of Offenses for "+this.rows[0].category.name;
                            this.categoryName = this.rows[0].category.name;
                            this.categoryID = this.rows[0].category.id;
                        }
